...
class Arena {
    ...
    static toggleAbility(hero, activated = false) {
        const chance = Math.random() * 100 <= 10;
        hero.abilities.forEach((ability) => {
            if (chance && !activated)
                console.log(hero.name + " activated the " + ability.type.toString() + " ability!");
            switch (ability.type) {
                case "Armour Boost":
    ...
    static hit(attacker, taker) {
        const isEvaded = Math.random() * 100 <= taker.evasion;
        if (isEvaded)
            console.log(taker.name + " evaded!");
        const initialDamage = attacker.weapon.usableBy.some((x) => x === attacker.type) && !isEvaded
            ? this.randomDamage(attacker.weapon.damage[0], attacker.weapon.damage[1])
            : 0;
        const finalDamage = initialDamage != 0 ? initialDamage - Math.floor(taker.armour / 3) : 0;
        taker.hp -= finalDamage;
        return initialDamage;
    }
    static consoleTable(firstHeroStarting, secondHeroStarting, firstHero, secondHero, firstAttacker, firstHeroRawDamage, secondHeroRawDamage) {
    ...
    static startTournament(firstHero, secondHero) {
        let round = 1;
        while (firstHero.hp > 0 && secondHero.hp > 0) {
            ...
            let firstAttacker = "";
            const firstHeroStarting = Object.assign({}, firstHero);
            const secondHeroStarting = Object.assign({}, secondHero);
            ...
            this.consoleTable(firstHeroStarting, secondHeroStarting, firstHero, secondHero, firstAttacker, firstHeroRawDamage, secondHeroRawDamage);
            round++;
        }
    }
}
